...
async def get_sources(
    ...
    try:
        ...
        if notebook_id:
            ...
            # Use proven subquery pattern from Notebook.get_sources()
            query = f"""
                SELECT *, 
...
async def create_source(
    ...
    try:
        ...
        if source_data.async_processing:
            ...
            if not source_title and upload_file and upload_file.filename:
                source_title = upload_file.filename
            if not source_title and file_path:
                source_title = os.path.basename(file_path)
            source_title = source_title or "Processing..."
